...
import {
  CreateUserArguments,
  TopMonitoringArguments,
  TopMonitoringDescriptionAndCountryArguments,
  UserArguments
} from '@/graphql/users/users.types';
import { requireRole, createNotFoundError } from '@/graphql/utils/errors';
...
export const userResolvers = {
  Query: {
    ...
    users: async (args: UserArguments, ctx: Context) => {
    ...
    userByEmail: async (args: UserArguments, ctx: Context) => {
    ...
    userById: async (args: UserArguments, ctx: Context) => {
      const user = requireRole(ctx, ['Admin', 'Manager', 'User']);
    ...
    topThreeUsersByMonitoring: async (
      args: TopMonitoringArguments,
      ctx: Context
    ) => {
  ...
  Mutation: {
    createUser: async (args: CreateUserArguments, ctx: Context) => {
  ...
  User: {
    role: async (parent: any, ctx: Context) => {
      return ctx.prisma.role.findUnique({ where: { id: parent.roleId } });
    },
    sessions: async (parent: User, ctx: Context) => {
      if (ctx.user.Role?.name === 'Manager' && ctx.user.id != parent.id) {
        return [];
      }
      return ctx.prisma.session.findMany({ where: { userId: parent.id } });
    },
    countries: async (parent: User, ctx: Context) => {
      return ctx.prisma.user.findUnique({ where: { id: parent.id } }).Country();
    },
    userMonitorings: async (parent: User, ctx: Context) => {
      if (ctx.user.Role?.name === 'Manager' && ctx.user.id != parent.id) {
        return [];
      }
