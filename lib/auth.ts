...
export async function getUserFromToken(req: NextApiRequest) {
    ...
    if (!session || !session.User) throw new Error("Acceso denegado: Sesión no válida.");

    if (session.User.email !== decoded.Email) {
        throw new Error("Acceso denegado: El correo del token no coincide con el del usuario.");
    }
